...
import { vectorOsmChangeGeneratorServerV1, type vectorOsmChangeGeneratorServerV1Type } from '@map-colonies/schemas';

// Choose here the type of the config instance and import this type from the entire application
type ConfigType = ConfigInstance<vectorOsmChangeGeneratorServerV1Type>;
...
async function initConfig(offlineMode?: boolean): Promise<void> {
  configInstance = await config({
    schema: vectorOsmChangeGeneratorServerV1,
    offlineMode,
  });
}
